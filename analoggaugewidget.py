...
class AnalogGaugeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        ################################################################################################
        self.setNeedleColor(100, 0, 0, 255)
        ...
        # DEFAULT VALUE COLOR
        ################################################################################################
        self.setDisplayValueColor(192, 192, 192, 255)
    ...
    def setGaugeTheme(self, Theme=1):
        ...
        if Theme == 100:
            self.set_scale_polygon_colors([[.1, Qt.red],
                                           [.15, Qt.yellow],
                                           [.35, Qt.green],
                                           [.6, Qt.yellow],
                                           [.7, Qt.red]])
    ...
    def updateValue(self, value, mouse_controlled=False):
        ...
        if value <= self.minValue:
            self.value = round(self.minValue, 2)
        elif value >= self.maxValue:
            self.value = round(self.maxValue, 2)
        else:
    ...
    def setGaugeColorOuterRadiusFactor(self, value):
        self.gauge_color_outer_radius_factor = int(value) / 1000
    ...
    def setGaugeColorInnerRadiusFactor(self, value):
        self.gauge_color_inner_radius_factor = int(value) / 1000
    ...
    def draw_big_scaled_marker(self):
        ...
        scale_line_lenght = float((self.widget_diameter / 2) -
    ...
    def create_scale_marker_values_text(self):
        ...
        for i in range(self.scalaCount + 1):
            ...
            text = str(round(float(self.minValue + scale_per_div * i),2))
            w = fm.width(text) + 1
            h = fm.height()
            painter.setFont(QFont(self.scale_fontname,
                            self.scale_fontsize, QFont.Bold))
            angle = angle_distance * i + \
                round(float(self.scale_angle_start_value - self.angle_offset),2)
            x = text_radius * math.cos(math.radians(angle))
            y = text_radius * math.sin(math.radians(angle))
            # print(w, h, x, y, text)

            painter.drawText(round(float(x - w / 2),2), round(float(y - h / 2),2), round(float(w),2),
                             round(float(h),), Qt.AlignCenter, text)
        # painter.restore()
    ...
    def create_fine_scaled_marker(self):
        ...
        my_painter.rotate(self.scale_angle_start_value - self.angle_offset)
        steps_size = (int(self.scale_angle_size) /
                      int(self.scalaCount * self.scala_subdiv_count))
        scale_line_outer_start = self.widget_diameter // 2
    ...
    def create_values_text(self):
        ...
        angle_end = int(self.scale_angle_start_value +
    ...
    def create_units_text(self):
        ...
        font = QFont(self.value_fontname, int(
            (self.value_fontsize+20) / 2.5), QFont.Bold)
        fm = QFontMetrics(font)
        ...
        painter.setFont(QFont(self.value_fontname, int(
            (self.value_fontsize+15) / 2.5), QFont.Bold))
    ...
    def mouseMoveEvent(self, event):
        ...
        if not x == 0:
            ...
            value = (int(math.fmod(angle - self.scale_angle_start_value + 720, 360)) /
                     (int(self.scale_angle_size) / int(self.maxValue - self.minValue))) + self.minValue
            temp = value
            fmod = int(
